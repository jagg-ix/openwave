...
def test_m9_registration_names_all_canonical_surfaces():
    ...
    assert M9_REGISTRATION.conformance_runner.endswith("model_conformance_dynamics:run_conformance_study")
    ...
    assert M9_REGISTRATION.formalization_import.endswith(":run_formalization_import_study")
    assert M9_REGISTRATION.canonical_spin_bridge.endswith(":run_canonical_spin_magnetic_bridge")
    assert M9_REGISTRATION.canonical_force_bridge.endswith(":run_canonical_force_formal_bridge")
    assert M9_REGISTRATION.charged_branch_feasibility.endswith(":run_charged_branch_feasibility")
    assert M9_REGISTRATION.charged_maxwell_source_bridge.endswith(":run_charged_maxwell_source_bridge")
    assert M9_REGISTRATION.field_force_triangle.endswith(":run_field_force_triangle")
    assert M9_REGISTRATION.current_evidence_authority.endswith(":run_current_evidence_authority")
    assert M9_REGISTRATION.calibration_ledger_v2.endswith(":run_physical_calibration_ledger_v2")
    assert M9_REGISTRATION.gauge_spinor_stationary_feasibility.endswith(
        "gauge_spinor_stationary_feasibility:run_gauge_spinor_stationary_feasibility"
    )
    assert M9_REGISTRATION.spinorial_pair_dynamics.endswith(
        "spinorial_pair_dynamics_current:run_spinorial_pair_dynamics"
    )
    assert M9_REGISTRATION.dynamics_formal_extension.endswith(":run_dynamics_formal_extension_study")
    assert M9_REGISTRATION.dynamics_evidence_authority.endswith(":run_dynamics_evidence_authority")
    assert M9_REGISTRATION.calibration_ledger_v3.endswith(":run_physical_calibration_ledger_v3")
...
def test_registration_carries_current_21_criterion_profile():
    ...
    assert payload["m9_97"]["gauge_spinor_stationary_campaign_passed"]
    assert payload["m9_97"]["full_pair_dynamics_campaign_passed"]
    ...
    assert payload["claim_boundary"]["four_spinor_sources_drive_initial_fields"]
