...
def test_full_registration_study_records_positive_and_negative_results():
    result = run_model_registration_study()
    assert result["schema"] == "openwave.model-registration.v4"
    ...
    # Historical registration is an evidence ledger. An intentionally negative
    # model result must remain represented rather than being converted into a
    # registration failure or a promoted physical claim.
    assert result["acceptance"]["charged_spinor_stationary_failure_is_registered"]
    assert result["acceptance"]["center_and_bmt_reductions_remain_open"]
    assert result["component_results"]["gauge_spinor_stationary_passed"]
    assert result["component_results"]["spinorial_pair_dynamics_passed"]
