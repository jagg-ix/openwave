# M5.22: nuclei as vortex knots (toy baryon census → supercomputer far arc)

**Status**: 🚧 PLANNED (staged 2026-07-18 from the author's autonomy goal list, goal (e); **TOY PHASE PULLED INTO 3×3 REACH 2026-07-19** by his 05:09 group message: "returning to 3x3 and studying simple vortex knots - which should correspond to proton and neutron ... with even toy simulations definitely deserving article"; [`m5_21_convo.md § 2026-07-19 05:09`](m5_21_convo.md)). This is the baryon/nuclei program (proton/neutron hunt), sequenced as its own arc after the M5.21 lepton series; the toy phase is un-gated by the 4D fork and parallel-safe with [M5.21.8](m5_21_8_task_details.md).

## Phase 1 (LIVE candidate): the 3×3 vortex-knot baryon census, toy
...
| Piece | Content | Notes |
| --- | --- | --- |
| Arena | The certified 3D instrument ([M5.21.2b](m5_21_2b_task_details.md) T2 sym stack), n ≥ 48; seed construction = hedgehog + surrounding vortex loop (proton analog) and the compensated arrangement (neutron analog) | 3×3, consumes delivered instruments only |
| Pre-registered reads | (i) do the knot seeds relax to protected minima (the baryon analogs exist)?; (ii) **mass ordering: neutron-analog > proton-analog** (his compensation argument); (iii) **charge-distribution profile: positive core / negative shell for the neutron-analog** vs Wilson, PhysRevLett 7, 144 (his experimental anchor); (iv) deuteron-analog: two-knot bound state + electric quadrupole moment sign | Each read is a falsifiable claim from his picture; report whatever comes |
...
## Phase 2 (far arc): supercomputer-scale nuclei

| Piece | Content | Notes |
| --- | --- | --- |
| Prerequisite | The Lagrangian details pinned by the e + ν programs ([M5.21.3](m5_21_3_task_details.md) → [M5.21.6](m5_21_6_task_details.md)/[M5.21.7](m5_21_7_task_details.md)) + the realistic-parameter bridge ([Q33](../m5_question_tracker.md#q33-detail)) | Quantitative fusion/fission claims need the bridge |
| Scale | He flags supercomputer-scale simulations; scoping (lattice sizes, solvers, partners) is itself a sub-task when the time comes | Far-gated |
| Experimental LC anchor | His public pointer (X, 2026-07-18): the Nature paper on fusion and fission of particle-like chiral nematic vortex knots, named as the style of nuclei model to build | Locate + file the paper into `theory/` at PLAN |
