...
What this establishes: the machinery does real regularized-soliton physics (the electron rest energy AND the fine-structure constant, Faber's two headline results), not "code from a training set." For the neutrino, two clean experiments pin down the structure: the plain disclination ring dissolves (run 1) and a smooth Hopfion expands (run 2), so the stable object is a **knotted/linked singular disclination** (singular core for the melt/`λ³`, knotting for protection) , Duda's chiral nematic vortex knot, now a precise, well-posed next build. _Figures: [`sandbox_v11/m5_11_p1_faber_electron.png`](../plots/m5_11_p1_faber_electron.png), [`sandbox_v11/m5_11_p1b_charge_137.png`](../plots/m5_11_p1b_charge_137.png)._
...
### P0 , the minimizer + V_M/LdG validation ✅ ([`sandbox_v11/m5_11_p0_minimizer.py`](../scripts/m5_11_p0_minimizer.py))
...
### P1a , FABER's electron reproduced ✅ ([`sandbox_v11/m5_11_p1_faber_electron.py`](../scripts/m5_11_p1_faber_electron.py), fig [`m5_11_p1_faber_electron.png`](../plots/m5_11_p1_faber_electron.png))
...
### P1b-foundation , the full 3D SU(2) curvature machinery ✅ ([`sandbox_v11/m5_11_p1b_lattice.py`](../scripts/m5_11_p1b_lattice.py))
...
### P1b , the fine-structure constant `α⁻¹≈137` ✅ ([`sandbox_v11/m5_11_p1b_dipole.py`](../scripts/m5_11_p1b_dipole.py), fig [`m5_11_p1b_charge_137.png`](../plots/m5_11_p1b_charge_137.png))
...
### P2 , the vortex loop: the plain ring dissolves (the fork) ⚠️ ([`sandbox_v11/m5_11_p2_vortex_loop.py`](../scripts/m5_11_p2_vortex_loop.py))
...
### Run 2 (2026-06-22 21:54) , Taichi-AD gradient engine ✅ ([`sandbox_v11/m5_11_ad_energy.py`](../scripts/m5_11_ad_energy.py))
...
### P2 (run 2) , the Hopfion test: a smooth knot does NOT stabilize ([`sandbox_v11/m5_11_p2_hopfion.py`](../scripts/m5_11_p2_hopfion.py))
...
| # | Spec | Thesis |
| --- | --- | --- |
| 1 | Add the chiral term `2 q0 L eps_ikl Q_ij d_k Q_lj` to the functional , **one more differentiable Taichi loop** in `m5_11_ad_energy.py` | Eq. 6 |
...
### P1b' , the quantitative running `α(d)`: machinery built, precise extraction needs Faber's high-order method ⚠️ ([`sandbox_v11/m5_11_p1b_running.py`](../scripts/m5_11_p1b_running.py))

Built a **differentiable Taichi version of the axisymmetric Faber energy** (the AD-engine port of the validated `m5_11_p1b_dipole.py` numpy energy) to attempt the two-soliton running `α(d)` (Faber Fig. 3) the quantitative way , a soliton+antisoliton pair, cores pinned at `±d/2`, projected-gradient AD-FIRE relaxation, `E₂(d)` fit to `A + B/d`, `α_sol = |B|`-derived.
...
### P2 BUILD (run 3, 2026-06-23) , the chiral + Frank terms built + validated; the biaxial M5 tensor will not host a clean smooth heliknoton 🔶 ([`sandbox_v11/m5_11_p2_heliknoton.py`](../scripts/m5_11_p2_heliknoton.py), fig [`m5_11_p2_heliknoton.png`](../plots/m5_11_p2_heliknoton.png))
...
### P2 SINGULAR DISCLINATION LOOP (run 4, 2026-06-23) , a single unknotted melted loop dissolves with OR without the chiral term: protection needs knotting/linking 🔶 ([`sandbox_v11/m5_11_p2_heliknoton.py`](../scripts/m5_11_p2_heliknoton.py) mode `disc`)
...
### P2 SINGULAR HOPFION (run 5, 2026-06-23) , an imposed melt on a smooth knot HEALS: the melt must be topologically FORCED, not painted on 🔶 ([`sandbox_v11/m5_11_p2_heliknoton.py`](../scripts/m5_11_p2_heliknoton.py) mode `shopf`)
...
| Artifact | What |
| --- | --- |
| [`sandbox_v11/m5_11_p0_minimizer.py`](../scripts/m5_11_p0_minimizer.py) | P0: FIRE + L-BFGS energy minimizer + the production functional in f64 numpy; gates gradcheck / vm_ldg / phi4 / hedgehog |
| [`sandbox_v11/m5_11_p1_faber_electron.py`](../scripts/m5_11_p1_faber_electron.py) | P1a: Faber radial soliton, generic seed → `arctan`, `I→π/4`, 511 keV |
| [`sandbox_v11/m5_11_p1b_lattice.py`](../scripts/m5_11_p1b_lattice.py) | P1b-foundation: full 3D SU(2) `Γ/R` curvature machinery, `O(a²)` convergence |
| [`sandbox_v11/m5_11_p1b_dipole.py`](../scripts/m5_11_p1b_dipole.py) | P1b: axisymmetric `(ρ,z)` energy (mode `single`) + charge quantization → `α⁻¹≈137` (mode `charge`) |
| [`sandbox_v11/m5_11_p2_vortex_loop.py`](../scripts/m5_11_p2_vortex_loop.py) | P2 (run 1): `+1/2` disclination-ring seeder + relax; the dissolve-vs-stabilize diagnostic |
| [`sandbox_v11/m5_11_ad_energy.py`](../scripts/m5_11_ad_energy.py) | run 2: Taichi-AD energy + gradient for the M5 functional, validated == P0 (the gradient engine) |
| [`sandbox_v11/m5_11_p2_hopfion.py`](../scripts/m5_11_p2_hopfion.py) | P2 (run 2): Hopf-map smooth-knot seed + AD-FIRE relax; shows a smooth knot expands |
| [`sandbox_v11/m5_11_p1b_running.py`](../scripts/m5_11_p1b_running.py) | P1b': differentiable axisym two-soliton AD energy + d-sweep; single-soliton validated, but `α_sol` not cleanly extractable in the fast setup |
| [`sandbox_v11/m5_11_p2_heliknoton.py`](../scripts/m5_11_p2_heliknoton.py) | P2 builds (runs 3-5): chiral Lifshitz + Frank terms added to the AD functional; modes `chiral_check` (validate), `calib` (helix-stationarity scan), `helix`, `sweep` (heliknoton A/B), **`disc`** (singular melted disclination loop ± chiral, run 4), **`shopf`** (singular Hopfion = Hopf charge + painted melt, run 5) |
| [`sandbox_v11/m5_11_p2_heliknoton_plot.py`](../scripts/m5_11_p2_heliknoton_plot.py) | P2 build figure generator (reads the two checkpoint JSONs) |
| [`sandbox_v11/m5_11_p1_faber_electron.png`](../plots/m5_11_p1_faber_electron.png), [`sandbox_v11/m5_11_p1b_charge_137.png`](../plots/m5_11_p1b_charge_137.png), [`sandbox_v11/m5_11_p2_heliknoton.png`](../plots/m5_11_p2_heliknoton.png) | P1a + P1b + P2-build figures |
| `sandbox_v11/_checkpoints/*.json` (`m5_11_p0_minimizer`, `m5_11_p1_faber_electron`, `m5_11_p1b_lattice`, `m5_11_p1b_axisym_single`, `m5_11_p1b_charge`, `m5_11_p2_vortex_loop`, `m5_11_p2_heliknoton`, `m5_11_p2_helix_calib`, `m5_11_p2_disclination_loop`, `m5_11_p2_singular_hopfion`) + `SESSION_STATE.md` | per-phase result checkpoints (small JSON, regenerable) |
