# M5.15: Composite particles (hopfions/knots to nuclei, atomic orbitals)

> Task **M5.15** (M5 / Liquid-Crystal model). Status: **Backlog** · Gated by: M5.9 foundations · Roadmap: [`m5_roadmap.md`](../m5_roadmap.md)

This doc is the task's full record: planning + findings + future planning + documentation.

---

## Current detail (from the roadmap, 2026-07-02 migration)

composites — its own research file (§5)

- **[15a — Composite Particles](15a_composite_particles.md)** — hopfions, knots → nuclei, atomic orbitals. Test program 15a.1 → 15a.5 (Liu et al. 2026 lab anchor). Pursued if foundations succeed and resources permit.
