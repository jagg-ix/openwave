...
| **Pre-registered topology read (his 2026-07-18 14:16 conjecture)** | "I suspect its four 1/2-vortices are usually connected forming two loops - hence usually emitting two neutrinos in muon/taon decay": count the CLOSED loops among the emitted structures (contour-winding instrument through the transition) and test four-half-lines-pair-into-two-loops | [`m5_21_convo.md § 2026-07-18 14:16`](m5_21_convo.md); sharpens the [Q31](../m5_question_tracker.md#q31-detail) closure residual |
| Render spec (his, same reply) | Any animation for him: SPARSE single-ellipsoid-per-angle glyphs emphasizing central behavior + vortices, not dense volumes (his Wolfram Community post as the template); split-vortex animations for μ and τ "especially from simulation" | Serves the deferred viz promise from the simulation endpoint, not as a separate viz task |
...
| 6 | **The released structure (descent grade): ONE thin equatorial ring** (ρ 15.2, z = 0, arc pair) visibly propagating out in the film, exited by the endpoint; the polar z ±12 features are A's own through-line run-outs, not emission. **His two-loop conjecture: count = 1 at descent grade, not 2** (descent quenches traveling structures; see #7) | note § 5, films |
| 7 | **Dynamics-grade (damped wave + sponge): the energy ledger CLOSES to 3 decimals** (E + KE + absorbed = 5.064 = start): **23% of the start energy radiated out through the sponge in t = 100**; **rotation-dominant transition** (rot 0.16° → 4.3°, spec_dev bounded, no melt spike) = his "quick field rotation" mechanism at dynamics grade; the biaxial object migrates out bodily (ρ 10 → 15) | note § 6 |
