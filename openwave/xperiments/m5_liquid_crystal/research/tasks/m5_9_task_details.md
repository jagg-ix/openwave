...
**After the M5.16 lock (2026-07-02 roadmap review): re-run the confiner test with the CALIBRATED potential.** The M5.9.3 scale-selection result (comment 4 below) used an **ad-hoc** core-volume confiner `E_conf = B·∫(1−s²)³ d³x` with a hand-fixed `B` to break scale-covariance and pin `r₀(Λ)`. Once M5.16 delivers the anchored LdG coefficients `(a, b, c)` at the physical `(g, δ)`, repeat the three-Λ minimization with the **real calibrated `V(M)`** and test whether scale-selection (the interior `E(r₀)` minimum) **comes for free** from the calibrated potential, or still needs a separate confiner term (itself a reportable structural finding about the potential). The eigenvalue-hierarchy origin (`1 : 5.9 : 15.1` under `E ∝ Λ³`) remains the honest irreducible frontier either way: the three-axes picture supplies the three-ness, the values stay Yukawa-like input until a geometric mechanism selects them. Cross-ref: [`m5_16_task_details.md`](m5_16_task_details.md) P-C/P-F.
