...
Working record of the multi-day exchange with Jarek Duda (Manfried Faber added mid-thread) that followed the M5.8 results report (`../archive/m5_summary_report.md`). Captures the correspondence and the runs it triggered, so the back-and-forth is preserved in one place alongside the findings-focused entry in `m5_roadmap.md` ("DUDA 2026-06-08 FOLLOW-UP" + its 2026-06-09 addendum).
...
**Cross-refs:** [`m5_roadmap.md`](../m5_roadmap.md) (DUDA FOLLOW-UP + 06-09 addendum, NG-12), [`m5_summary_report.md`](../archive/m5_summary_report.md), `sandbox_vn/m5_8_2q_delta_scaling.py` (energy sweep + Phase D/E boost/EM-GEM split), `sandbox_vn/m5_8_2q_omega.py` (ω(δ) scaffold, deferred), `../theory/faber_universe_2025.pdf`.
