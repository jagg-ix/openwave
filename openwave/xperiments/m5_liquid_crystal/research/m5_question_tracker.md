...
```text
3 RE-ASK      Q13/Q14/Q15, re-asked 2026-07-04 (the M5.17 email: method
(email out    note first + the melt-channel evidence). His 2026-07-03b
2026-07-04)   reply independently re-confirmed regularization + potential
              details as the hard core, backing exactly this set:
                Q13  chiral (Lifshitz) invariant: a quadratic gradient
                     term is the class that would CLOSE the melt channel
                Q14  hedgehog core point-vs-ring: what holds point
                     defects/pairs (melt channel measured open ~2 orders)
                Q15  delta-pinning / sixth-order: would also raise the
                     melt cost (same channel), beyond the 3b*delta
                     residual
...
1 HELD        Q17 beta and g anchors: held for the NEXT round (avoid
(next round)  overload; the paper's own p.11 hints delta^2 ~ hbar*c,
              g^4 ~ 1e38 logged in m5_9_task_details; clock frequency
              is on his electron checklist = the clock/boost path)
...
Total: 17 questions (3 re-asked, 1 banked partial, 1 held, 1
self-determine, 4 background, 7 resolved). The ask queue itself is baked
into tasks/m5_12_task_details.md (the consumer); this file = the registry.
Sequence now: M5.17 DONE (module + method note + two-charge) -> the
re-ask email (out 2026-07-04) -> Q13/Q14/Q15 answers feed phases A/C/D
-> go M5.12. delta_CP fork = framing under Q13.
```
...
| [Q14](#q14-detail) 🔶 | Hedgehog core: point vs ring | The spherical hedgehog is a saddle AND the melt-line channel is measured open ~2 orders (M5.17): what holds point defects/pairs (Frank term? sixth-order? clock dressing?), or are the escaped textures the intended particles? (re-asked 2026-07-04) | electron ground state; Q8 residual; M5.12 phase D |
| [Q15](#q15-detail) 🔶 | δ-pinning | The quartic trace-LdG cannot be stationary at `(1, δ, 0)` (residual force `3bδ`) and its cheap melt leaves the channel open: sixth-order invariant intended, or is δ dynamical? (re-asked 2026-07-04) | Q7 residual; vacuum structure; M5.9 |
...
| [Q17](#q17-detail) 🔶 | β and g anchors | Preferred anchors for the two open lock slots: β = b/c (via `κ_δ = (3/2)b`) and g (statics measured g-blind: clock/boost vs baryon mass)? (HELD for the next round) | full `(g, δ, a, b, c)` lock; `#220`; M5.9 |
