...
```text
5 ASK ROUND   ASKED 2026-07-02 (the M5.16 email); Duda's 2026-07-03 reply
(sent; reply  audited the METHODS instead of answering ("I don't see [the
= methods     potential / Hamiltonian] ... no idea what does it calculate";
audit; re-ask tasks/m5_4h_convo_2026.07.03.md). RE-ASK rides the M5.17
rides M5.17)  methods note. Per-ID state:
                Q13  chiral (Lifshitz) invariant: UNANSWERED, re-ask
                Q16  neutrino seed topology: PARTIAL, banked (single
                     vortex "rotated cylindrically to make it loop"
                     first; pair-vs-trefoil discrimination still open)
                Q14  hedgehog core point vs ring: UNANSWERED (his Fig. 9
                     ansatz reference sharpens the target; M5.17 checks
                     conformance)
                Q15  delta-pinning: UNANSWERED ("search for parameters"
                     framing keeps it live)
                Q17  beta and g anchors: weak signal only (clock
                     frequency is on his electron checklist)
...
Sequence now: M5.17 (methods note + two-charge Coulomb, per
dev_docs/METHOD_NOTE.md) -> the re-ask email leads with the note ->
answers feed phases A/C/D -> go M5.12. delta_CP fork = framing under Q13.
```
...
| [Q16](#q16-detail) 🔶 | Neutrino seed topology | Which object is the neutrino: a Hopf-linked `+1/2` disclination pair, a `+1/2` trefoil knot, or the `4f` sketch's "two vortex types" composite? (2026-07-03 partial: single rotated vortex loop FIRST) | M5.12 phases A/C seeds |
