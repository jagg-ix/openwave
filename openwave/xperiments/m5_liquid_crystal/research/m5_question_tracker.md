...
2 IN-TEST     Q14 hedgehog core / what holds defects: NOT answered in
(M5.18 runs   words, but his universal potential makes the melt-channel
the answer)   verdict COMPUTABLE (melt now costs V(0) = sum_p c_p^2 per
              cell vs the quartic's tiny c - b/2) -> M5.18 phase B
              re-measures the channel; either outcome is reportable
              Q15 delta-pinning: ANSWERED practically 2026-07-05: the
              universal spectral potential V(M) = sum_p (Tr(M^p)-c_p)^2,
              c_p = sum_i Lambda_i^p, targets (1,delta,0) 3D /
              (g,1,delta,0) 4D; pins the biaxial vacuum EXACTLY.
              Residual (dynamical-delta intent + coefficient freedom)
              rides M5.18 phase B; "delta=0 = without QM" logged
...
Total: 17 questions (1 pending his side, 2 in-test via M5.18, 1 banked
partial, 1 held, 1 self-determine, 4 background, 7 resolved). NEW INBOUND
2026-07-05 (his second reply, tasks/m5_17_convo.md entry 2): the 4D
Lagrangian + a verification request delegated to the agent BY NAME
("nobody else has checked it") + two back-questions on the two-charge
diagram -> all consumed by task M5.18 (gates M5.12).
Sequence now: M5.18 (verify 4D Lagrangian/Hamiltonian + swap in the
universal potential + gate suite + melt-channel verdict) -> reply email
(deliverable + the fixed-ansatz answer) -> go M5.12 on the new
instrument. delta_CP fork = framing under Q13.
```
...
| [Q14](#q14-detail) 🔶 | Hedgehog core: point vs ring | The spherical hedgehog is a saddle AND the melt-line channel is measured open ~2 orders (M5.17): what holds point defects/pairs? NOW IN-TEST: under the 2026-07-05 universal potential the melt cost jumps to `V(0) = Σ_p c_p²`, so M5.18 phase B MEASURES whether the channel closes | electron ground state; Q8 residual; M5.18 phase B; M5.12 phase D |
