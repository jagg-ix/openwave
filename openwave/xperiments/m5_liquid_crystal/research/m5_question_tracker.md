...
```text
1 PENDING     Q13 chiral (Lifshitz) invariant: the one re-asked question
(his side)    still fully open after his 2026-07-05 second reply
              ("will study further and write" remains the standing state)

2 MEASURED    Q14 hedgehog core / what holds defects: M5.18 phase B RAN
(M5.18 done,  the test on his universal potential: THE CHANNEL SURVIVES
words         (hedgehog still escapes 55%, antipair still annihilates,
pending)      melt bridge identical): no spectrum-pinning potential
              shape closes it; candidates left = the Q13 quadratic-
              gradient pair or the clock dressing. His in-words answer
              (re-asked 2026-07-04) still pending
              Q15 delta-pinning: ANSWERED 2026-07-05 (the universal
              spectral potential) AND implemented + validated same day:
              exact biaxial pinning machine-confirmed; r_half = 2.935 fm
              within 0.3% of the LdG value (potential-shape ROBUST).
              Residual: dynamical-delta intent + per-p weight freedom;
              "delta=0 = without QM" logged
...
(next round)  slot DISSOLVES with the potential swap (no more (a,b,c));
              remaining anchors = the g VALUE (enters via the c_p
              spectrum targets) + the overall energy scale (m_e as
              before); p.11 hints delta^2 ~ hbar*c, g^4 ~ 1e38 now read
              as c_p coefficient constraints; 6.2 pm = the loop-size
              candidate; still held, next round
...
7 RESOLVED    Q1  initial-condition construction (no static soliton,
              ...
              Q7  V(M) coeffs: DELIVERED by M5.16, locked modulo beta
                  (residual -> Q15; instrument superseded 2026-07-05 by
                  the universal spectral potential, M5.18)
              Q8  regularization: DELIVERED by M5.16, melt profile s(r),
                  virial 1.006 (residual -> Q14)

Total: 17 questions (1 pending his side, 2 measured via M5.18 (words
pending), 1 banked partial, 1 held, 1 self-determine, 4 background,
7 resolved). M5.18 DELIVERED 2026-07-05: his 4D Lagrangian VERIFIED
(Lorentz-invariant + Legendre-correct) with 3 qualifications
(constraints; covariant vacuum diag(-g,1,delta,0); H indefinite:
negative-energy boost textures ON the vacuum manifold + the 4-branch
vacuum split -> NEW owner-intent questions for the reply email);
his potential implemented + validated (r_half potential-shape robust).
Sequence now: reply email (the verification deliverable + the
fixed-ansatz answer + the boost-texture/branch questions) -> his
Q13/Q14/Q15 words -> go M5.12 on the new instrument.
delta_CP fork = framing under Q13.
```
...
| --- | --- | --- | --- |
| [Q13](#q13-detail) 🔶 | Chiral invariant | Does the M5 LdG substrate carry a chiral (Lifshitz) invariant `2q₀L ε_ikl M_ij ∂_k M_lj` + Frank partner, or is achirality intended? (re-asked 2026-07-04: the class of term that would close the measured melt channel) | M5.12 phases A-C, F; the CP sector |
| [Q14](#q14-detail) 🔶 | Hedgehog core: point vs ring | What holds point defects/pairs? MEASURED on TWO potentials (M5.17 LdG + M5.18 spectral): the melt channel survives ANY spectrum-pinning potential shape (hedgehog escapes, antipair annihilates, both forms); candidates left = the Q13 quadratic-gradient pair or the clock dressing. His in-words answer still pending | electron ground state; Q8 residual; M5.12 phase D |
