...
| [`m5_question_tracker.md`](m5_question_tracker.md) | author-gated unknowns behind rows marked 🔶 (currently: Q24 DEFERRED BY AUTHOR 2026-07-15: the least-action/breathing question is with him, "can elaborate ... seems quite difficult", program redirected to the electron hedgehog; the γ = −1 ask was withdrawn at M5.20.5; the tu → s calibration). NOTE two author directives from the same reply, to be VERIFIED at [M5.21.1](tasks/m5_21_1_task_details.md) before entering this registry: the `(-g)^p` potential-target correction (sign of g open, both branches to be gated + regressed) and the physical scales delta ~ 1e-10, g ~ 1e10 (our locked toy values stand as the sanctioned "practical approximation" pending the scaling ladder) |
