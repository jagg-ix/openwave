...
M[0,0] = g (time axis, frozen). Energy/gradient/minimizer reused from m5_11_p0_minimizer.
...
Run:  python m5_11_p2_vortex_loop.py [N] [R0] [iters]
"""
...
def main():
    ...
    with open(os.path.join(CKPT, "m5_11_p2_vortex_loop.json"), "w") as f:
        json.dump(out, f, indent=2)
    print(f"\nP2 verdict: {verdict.upper()}  (R {R_init:.2f} -> {R_final:.2f} vox, "
          f"{done} iters, {round(time.time()-t0,1)}s)")
    print(f"checkpoint -> {os.path.join(CKPT, 'm5_11_p2_vortex_loop.json')}")
