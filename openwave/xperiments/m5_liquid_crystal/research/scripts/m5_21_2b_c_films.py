"""M5.21.2b films: 3D endpoints on the TRUE m5_film.py templates
(basic + thermal), the series film rule (the M5.21.2 review catch,
closed here).

Same embed as m5_21_2_e_film_adapter.py: the y = 0 meridional
half-plane slice (rho = x >= 0, z), 3x3 embedded 4x4 block-diag with
the DISPLAY-CONSTANT time eigenvalue g (the 3D stack has no time
row; the constant row is labeled in the suptitle). The basic
template's energy-density panel gets the TRUE 3D density (this
task's instrument: sym stencil + eps-Dirichlet + term set) via the
density_fn hook; the thermal template renders its standard panels on
the embedded states.

Run:  python3 m5_21_2b_c_films.py <endpoint_npz> <tag> [seedkind]
Out:  ../plots/m5_21_2b_film_basic_<tag>.png + _thermal_
"""
import os
import runpy
import sys
...
def dens3_slice(M3, cfg):
    """true 3D density under THIS run's instrument, y = 0 slice."""
    h = cfg["h"]
...
def main(path, tag, seedkind="A"):
    ...
    film_strip(states, os.path.join(
        PLOTS, f"m5_21_2b_film_basic_{tag}.png"),
        template="basic", delta=delta, h=h, g=G_DISP,
        wscale=g["W1"], density_fn=density_fn,
        suptitle=f"M5.21.2b {tag} (3D {term} {cfg['stencil']} "
                 f"eps={eps:g}) on the TRUE basic template "
                 f"(y = 0 half-plane; time row = display constant g)")
    film_strip(states, os.path.join(
        PLOTS, f"m5_21_2b_film_thermal_{tag}.png"),
        template="thermal", delta=delta, h=h, g=G_DISP,
        suptitle=f"M5.21.2b {tag} on the TRUE thermal template "
                 f"(y = 0 half-plane; time row = display constant "
                 f"g, 3D statics)")
