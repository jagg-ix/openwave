...
def mode_euclid():
    ...
    np.savez(HERE.parent / "data" / "_m5_8_2e_euclid.npz", **{k: v for k, v in R.items() if k != "onset"})
    print(
...
def mode_omega():
    ...
    np.savez(HERE.parent / "data" / "_m5_8_2e_omega.npz", **{k: v for k, v in R.items() if k != "onset"})
    s = R["s_dense"]
    if len(s) > 1000:
