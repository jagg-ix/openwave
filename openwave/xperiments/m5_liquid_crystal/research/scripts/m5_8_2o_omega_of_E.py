...
def out_npz(k):
    return HERE.parent / "data" / f"_m5_8_2o_k{k:g}.npz"
...
def analyze_main():
    ...
    zh = dict(np.load(HERE.parent / "data" / "_m5_8_2h_dense.npz"))      # the high-E arms
...
def settle_main():
    ...
    np.savez(HERE.parent / "data" / "_m5_8_2o_settled.npz", M_settled=M_set,
...
def restart_main():
    ...
    zs = np.load(HERE.parent / "data" / "_m5_8_2o_settled.npz")
    M_set = zs["M_settled"]
    actb = act > 0.5
    n_act = int(act.sum())
    inv2h = 1.0 / (2.0 * h)
    ...
    np.savez(HERE.parent / "data" / "_m5_8_2o_ground.npz", p2=np.array(p2s),
             umin=np.array(umins), ht=np.array(hts), hH=np.array(hHs),
