...
Run:  python m5_11_p2_heliknoton.py [mode] [N] [pitch] [iters]
"""
...
@ti.kernel
def compute_energy():
    # --- curvature: 3 loops (per axis-pair), validated == P0 in m5_11_ad_energy.py ---
...
def run_sweep():
    ...
    with open(os.path.join(CKPT, "m5_11_p2_heliknoton.json"), "w") as f:
        json.dump(out, f, indent=2)
    print(f"\n  chiral+Frank stabilizes a localized knot (control L=0 does not): {chiral_helps} "
          f"(L=0 -> {r0:.2%}, best -> {rmax:.2%}, n_holds={len(holds)})")
    print(f"  checkpoint -> {os.path.join(CKPT, 'm5_11_p2_heliknoton.json')}")
    return out
...
def run_calib():
    ...
    with open(os.path.join(CKPT, "m5_11_p2_helix_calib.json"), "w") as f:
    ...
    print(f"  checkpoint -> {os.path.join(CKPT, 'm5_11_p2_helix_calib.json')}")
...
def run_disc():
    ...
    with open(os.path.join(CKPT, "m5_11_p2_disclination_loop.json"), "w") as f:
        json.dump(out, f, indent=2)
    print(f"\n  chiral protects the loop (control L=0 does not hold): {chiral_helps}")
    print(f"  checkpoint -> {os.path.join(CKPT, 'm5_11_p2_disclination_loop.json')}")
    return out
...
def run_shopf():
    ...
    with open(os.path.join(CKPT, "m5_11_p2_singular_hopfion.json"), "w") as f:
        json.dump(out, f, indent=2)
    print(f"\n  the melt stabilizes the Hopfion (smooth expands, melted holds): {melt_stabilizes}")
    print(f"  checkpoint -> {os.path.join(CKPT, 'm5_11_p2_singular_hopfion.json')}")
    return out
