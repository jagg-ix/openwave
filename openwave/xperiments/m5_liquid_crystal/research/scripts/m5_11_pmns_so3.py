#!/usr/bin/env python3
"""m5_11_pmns_so3 — PMNS neutrino mixing from the SO(3) rotation structure.
...
swinging WITHOUT the hedgehog winding (Duda's Wolfram slide, m5_roadmap.md L786).
...
summary = {
    ...
    "delta_CP_test": {
        "so3_requires_deg": 180.0,
        "measured_deg": nufit["dCP"],
...
with open(os.path.join(DATA, "m5_11_pmns_summary.json"), "w") as f:
...
ax[1].axvspan(
    nufit["dCP_range_1sig"][0],
    nufit["dCP_range_1sig"][1],
    color="#e67e22",
    alpha=0.3,
...
ax[1].set_title(
    "(b) The falsifiable δ_CP = 180° test\nCONSISTENT (best fit ~177°); JUNO/DUNE will sharpen"
)
ax[1].legend(fontsize=9, loc="upper right")

plt.tight_layout()
plt.savefig(os.path.join(PLOTS, "m5_11_pmns_so3.png"), dpi=130)
plt.close()
...
print("=" * 72)
print("m5_11 — PMNS FROM THE SO(3) ROTATION STRUCTURE (#199)")
print("=" * 72)
print(
    "SO(3)/TBM prediction:  theta12=%.2f  theta13=%.1f  theta23=%.1f  dCP=%.0f"
    % (t12_tbm, t13_tbm, t23_tbm, dCP_tbm)
)
print(
    "NuFIT 6.0 (measured):  theta12=%.2f  theta13=%.2f  theta23=%.1f  dCP=%.0f"
...
print(
    "  theta23 ~ maximal (45):  TBM 45  vs meas %.1f/%.1f  -> straddles 45 ✓"
    % (nufit["theta23_lower"], nufit["theta23"])
)
print(
    "  theta12 ~ tri-maximal:   TBM 35.26 vs meas %.1f  (off %.1f deg) ✓"
    % (nufit["theta12"], nufit["theta12"] - t12_tbm)
)
print(
    "  theta13 = SO(3)-breaking: TBM 0 -> meas %.1f deg = the REQUIRED second rotation (SMALL)"
    % nufit["theta13"]
)
print(
    "  dCP = 180 (SO(3)):       meas ~%.0f, consistent at 1sigma: %s"
...
print(
    "  naive sin^2(theta13) = %.4f (2.2%%) = ~%.0fx too big -> electron clock is TIGHTER-projected"
    % (f_off_naive_theta13, ratio_naive_to_obs)
)
print("  structural match to Duda's reading; specific number = the dynamical follow-up")
print("=" * 72)
print("Wrote data/m5_11_pmns_summary.json + plots/m5_11_pmns_so3.png")
