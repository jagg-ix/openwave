...
| --- | --- | --- | --- |
| (none) | | M6.1 is staged first (small, day-scale); M6.2 is the decision gate behind it | |

## BACKLOG (the active, decidable core: frozen-spec work)

| TaskID | Title | Description | Gated By |
| --- | --- | --- | --- |
| M6.1 | The spec certification gate (small) | Two arms, both cheap. (a) FREEZE THE TARGET: pin the validation spec to v11 (Zenodo 20357670) with its exact normalization, resolving the notation drift across records (canonical § 2) into one written convention sheet the later tasks pre-register against. (b) CHARACTERIZE v4 for the record: verify what is checkable in a day (EL equations, Hamiltonian boundedness, dimensional analysis, the λ symbol collision, the 0.460 vs 1.033 MeV mass split) and document that the constraint C[φ] is unspecified, certifying the pre-analysis pin decision in a citable form. Deliverable: method note + canonical § 1/§ 2 updates. | user "go" |
| M6.2 | THE DECISION GATE: H/Q under pre-registered conventions | The electron benchmark carries the corpus's heaviest provenance defects (canonical § 4: target drift, variant-search reproduction, the author's own July 8 window-defined downgrade). Re-derive H/Q from the frozen v11 spec with EVERY convention fixed in advance: the Q_CS normalization DERIVED (not selected to match), the H functional stated term-by-term, the ansatz pinned to the papers' production form, the quartic term's inclusion decided by derivation, NO search. Report the number against both candidate targets (physical 1.6875; model-internal 1.6969). **Branch (a), survives**: the electron cell is re-earned, M6.3/M6.4 proceed. **Branch (b), fails**: the electron sector is closed honestly (MODELS.md + briefing + hunt synced), and M6 returns to hold with the DM sector + M7 lineage as the durable record; only M6.4 remains optionally live. Cross-check either way: the M7 full-3D window-defined measurement (4.7e-5 code-to-code). | M6.1 + user "go" |
...
| Question | Answer |
| --- | --- |
| Where is M6? | REFRESHED from "permanent hold" and RE-SCOPED same day (pre-analysis, user-approved): close the ledger on the frozen v11 spec; do not chase the July-era moving target. Active core = M6.1 → M6.2 (decision gate) → M6.3/M6.4; July-era programs parked with a named reopening condition |
