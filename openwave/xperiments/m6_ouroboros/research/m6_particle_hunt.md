...
| Muon / tau | higher-ω states of the same couplings: ω = 11.0 / 40.7 (May 16, g = 1.0625) then 12.82 / 50.0 (May 23, g = 1.0) then "(Ω, G) bifurcation islands" (June 29, no mass values) | ❌ rides the invalidated H/Q machinery (M6.2), and the audit's far-field analysis excludes localized charged states for every ω above 0.786, so the whole ω ladder is window-defined; the ω values were scan-fitted with the discreteness mechanism admitted open. The discreteness QUESTION stays probeable (M6.4b) | silent |
| Pion+ | chaoiton at ω = 15.0, 3.25% from 139.57 MeV | ❌ same invalidated machinery and non-localized state family as the lepton ladder (M6.2); π⁰ (ω = 0) was unexplained in any era | silent |
