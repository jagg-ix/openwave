"""README/MODELS-aligned CAT/EPT conformance profile through M9.68."""
...
CRITERIA: tuple[Criterion, ...] = (
    ...
    Criterion("de_broglie_clock", "particles", "de Broglie clock (Zitterbewegung)", "partial", (ROOT + "intrinsic_clock_reduction.py", ROOT + "imaginary_action_backreaction.py", ROOT + "entropic_integrator.py", ROOT + "physical_calibration_ledger.py", ROOT + "preregistered_breathing_prediction.py", ROOT + "independent_breathing_comparison.py", FINDINGS + "m9_65_method_note.md", FINDINGS + "m9_68_method_note.md", FORMAL_STATUS), "PhysLib proves a scoped operational entropic/physical proper-time equality under its explicit action-rate calibration. The frozen M9.65 Gaussian breathing prediction fails the no-refit M9.68 spatial comparison by 43--49 percent. This rejects that collective-coordinate subprediction, not every CAT/EPT clock channel or a physical Zitterbewegung model."),
    Criterion("particle_stability", "particles", "Particle stability (Derrick escape)", "partial", (ROOT + "minimizer_3d.py", ROOT + "stability_campaign_3d.py", ROOT + "unified_self_binding_3d.py", ROOT + "non_gaussian_self_binding.py", ROOT + "action_derived_binding.py", ROOT + "binding_axiom_selection.py", ROOT + "orbital_compactness_bridge.py", ROOT + "coefficient_self_consistency.py", ROOT + "cubic_quintic_continuum.py", ROOT + "selection_condition_derivation_audit.py", ROOT + "h1_orbital_adversarial.py", FINDINGS + "m9_49_method_note.md", FINDINGS + "m9_52_method_note.md", FINDINGS + "m9_59_method_note.md", FINDINGS + "m9_60_method_note.md", FINDINGS + "m9_61_method_note.md", FINDINGS + "m9_63_method_note.md", FINDINGS + "m9_64_method_note.md", FINDINGS + "m9_66_method_note.md", FINDINGS + "m9_67_method_note.md"), "The original action disperses. M9.66 derives Gaussian scale stationarity but rejects peak-density matching as the current full-field first-principles condition. M9.67 keeps six adversarial perturbation classes bounded on two grids and respects the coercive gradient bound. A stationary non-Gaussian branch, kernel H1 well-posedness, arbitrary-H1 orbital stability, and a physical particle remain open."),
...
def canonical_payload() -> dict[str, Any]:
    return {"schema": "openwave.m9.models-conformance.v5", "model": "M9 CAT/EPT", "criteria": [asdict(item) for item in CRITERIA], "audit": validate_profile()}
...
def run_conformance_study() -> dict[str, Any]:
    ...
    acceptance = {
        ...
        "m9_68_status_counts": payload["audit"]["status_counts"] == {"validated": 0, "partial": 20, "negative": 1, "not_yet": 0},
        "m9_66_67_stability_evidence_present": all(any(name in path for path in stability["evidence"]) for name in ("m9_66_method_note.md", "m9_67_method_note.md")),
        "m9_68_falsification_evidence_present": any("m9_68_method_note.md" in path for path in clock["evidence"]),
        "falsified_subprediction_not_promoted_to_criterion_negative": clock["status"] == "partial",
    }
