"""README/MODELS-aligned CAT/EPT conformance profile through M9.71."""
...
CRITERIA: tuple[Criterion, ...] = (
    ...
    C("de_broglie_clock", "particles", "de Broglie clock (Zitterbewegung)", "partial", (ROOT + "intrinsic_clock_reduction.py", ROOT + "imaginary_action_backreaction.py", ROOT + "entropic_integrator.py", ROOT + "physical_calibration_ledger.py", ROOT + "preregistered_breathing_prediction.py", ROOT + "independent_breathing_comparison.py", ROOT + "replacement_mode_prediction.py", FINDINGS + "m9_65_method_note.md", FINDINGS + "m9_68_method_note.md", FINDINGS + "m9_71_method_note.md", FORMAL_STATUS), "PhysLib proves a scoped operational entropic/physical proper-time equality under its explicit action-rate calibration. The frozen M9.65 Gaussian breathing prediction fails the no-refit M9.68 spatial comparison. M9.71 freezes a stationary-branch radial ratio of 1.074356835825 and passes internal held-out 24-cubed and 28-cubed grids without refitting. No external experiment or physical Zitterbewegung identification is established."),
    C("particle_stability", "particles", "Particle stability (Derrick escape)", "partial", (ROOT + "minimizer_3d.py", ROOT + "stability_campaign_3d.py", ROOT + "unified_self_binding_3d.py", ROOT + "non_gaussian_self_binding.py", ROOT + "action_derived_binding.py", ROOT + "binding_axiom_selection.py", ROOT + "orbital_compactness_bridge.py", ROOT + "coefficient_self_consistency.py", ROOT + "cubic_quintic_continuum.py", ROOT + "selection_condition_derivation_audit.py", ROOT + "h1_orbital_adversarial.py", ROOT + "stationary_non_gaussian_branch.py", ROOT + "h1_kernel_certificate.py", FINDINGS + "m9_49_method_note.md", FINDINGS + "m9_52_method_note.md", FINDINGS + "m9_59_method_note.md", FINDINGS + "m9_60_method_note.md", FINDINGS + "m9_61_method_note.md", FINDINGS + "m9_63_method_note.md", FINDINGS + "m9_64_method_note.md", FINDINGS + "m9_66_method_note.md", FINDINGS + "m9_67_method_note.md", FINDINGS + "m9_69_method_note.md", FINDINGS + "m9_70_method_note.md"), "The original action disperses and M9.66 rejects peak-density matching as a first-principles rule. M9.69 constructs one localized non-Gaussian stationary branch from the full normalized spatial equation across unrelated seeds and nested grids. PhysLib proves exact cubic-quintic density coercivity and a conditional uniform orbital theorem from explicit H1 flow, conservation, compactness, and Lyapunov certificate data. Construction of those analytic PDE certificates and identification of a physical particle remain open."),
...
def canonical_payload() -> dict[str, Any]:
    return {"schema": "openwave.m9.models-conformance.v6", "model": "M9 CAT/EPT", "criteria": [asdict(item) for item in CRITERIA], "audit": validate_profile()}
...
def run_conformance_study() -> dict[str, Any]:
    ...
    acceptance = {
        ...
        "m9_71_status_counts": payload["audit"]["status_counts"] == {"validated": 0, "partial": 20, "negative": 1, "not_yet": 0},
        "m9_69_70_stability_evidence_present": all(any(name in path for path in stability["evidence"]) for name in ("m9_69_method_note.md", "m9_70_method_note.md")),
        "m9_71_replacement_mode_evidence_present": any("m9_71_method_note.md" in path for path in clock["evidence"]),
