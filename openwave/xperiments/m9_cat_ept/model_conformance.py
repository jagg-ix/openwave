...
def validate_profile(criteria:tuple[Criterion,...]=CRITERIA)->dict[str,Any]:
    ...
    domains={d:sum(x.domain==d for x in criteria) for d in ("particles","forces","waves")}; counts={s:sum(x.status==s for x in criteria) for s in ("validated","partial","negative","not_yet")}
...
def run_conformance_study()->dict[str,Any]:
    payload=canonical_payload(); acceptance={"all_explicit_rows_covered":payload["audit"]["criterion_count"]==20,"particle_force_wave_partition_closes":payload["audit"]["domain_counts"]=={"particles":12,"forces":5,"waves":3},"nonplanned_cells_have_evidence":all(x["status"]=="not_yet" or bool(x["evidence"]) for x in payload["criteria"]),"honest_negative_preserved":any(x["status"]=="negative" for x in payload["criteria"]),"summary_mismatch_recorded":payload["audit"]["matrix_total_mismatch"]==1,"thermal_gap_named":payload["audit"]["missing_explicit_criterion"]==MISSING_EXPLICIT_CRITERION,"deterministic_fingerprint":fingerprint()==fingerprint(),"m9_34_36_status_counts":payload["audit"]["status_counts"]=={"validated":0,"partial":13,"negative":1,"not_yet":6}}
