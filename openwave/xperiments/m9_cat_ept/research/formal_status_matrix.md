...
Lean remains proof authority. ZIL remains the dependency, source-traceability, status, rule, and query graph.
...
| Global conservative certificate, compact minimizing orbit, and identified branch | directly present / adapted | physical identity remains separate |
...
| M9.94 canonical spin-magnetic bridge | OpenWave executable | tree `g=2`; anomalous moment and electron identity remain open |
| M9.95 canonical electric/magnetic force bridge | OpenWave executable | declared winding pair; charged stationary branches and units remain open |
...
The platform matrix remains `7 validated / 13 partial / 1 negative`. Formal availability does not promote magnetic moment, electric force, magnetic force, gravity, calibration, particle identity, or external-evidence rows.
