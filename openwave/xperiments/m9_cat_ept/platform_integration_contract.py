"""Fail-closed platform contract for the stable M9.126 aliases and later M9 work."""
...
SCHEMA = "openwave.m9.platform-integration-contract.v9"
...
def run_platform_integration_contract() -> dict[str, Any]:
    ...
    payload = {
        ...
        "claim_boundary": {
            "existing_evidence_registry_implies_prospective_validation": False,
            "broad_planckian_band_implies_unique_cat_ept_support": False,
        },
    }
    profile = docs["MODELS_M9.md"]
    roadmap = docs["openwave/xperiments/m9_cat_ept/research/m9_roadmap_maturity.md"]
    acceptance = {
        "stable_registration_points_to_schema_v29": (
            registration["schema"] == CURRENT_REGISTRATION_SCHEMA
        ),
        "stable_registration_points_to_current_conformance": (
            registration["registration"]["conformance_runner"]
            == CURRENT_CONFORMANCE_RUNNER
        ),
        "stable_conformance_preserves_schema_v22": (
            conformance["schema"] == CURRENT_CONFORMANCE_SCHEMA
        ),
        "stable_conformance_composes_M9_126": (
            conformance["current_milestone"] == CURRENT_MILESTONE
            and conformance["latest_evidence"]["passed"]
        ),
        "public_documents_expose_the_m9_program": all(
            "M9" in text for text in docs.values()
        ),
        "profile_names_current_schemas_and_evidence_boundary": all(
            token in profile
            for token in (
        ...
        "roadmap_retains_future_work_and_next_marker": (
            "NEXT" in roadmap and any(marker in roadmap for marker in ("M9.127", "M9.13"))
        ),
        "existing_evidence_is_registered_without_promotion": (
            current["existing_evidence_qualified"]
            and not current["prospective_external_validation_complete"]
    ...
    return {
        **payload,
        ...
        "decision": {
            ...
            "later_milestones_may_extend_the_stable_alias": True,
