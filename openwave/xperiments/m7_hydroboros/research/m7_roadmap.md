...
| [M7.10](tasks/m7_10_pure_maxwell.md) | Pure-Maxwell sector: the no-Lagrangian test | switch the coupling terms off (`m_J² A·J` and `f(J·J)` zero, leaving plain Maxwell evolution `∂E/∂t = ∇×B`, `∂B/∂t = −∇×E`; the Ouroboros vortex theme stays, only the Lagrangian machinery is removed); four experiments: E1 Trkalian cavity control (CK mode + Dirichlet shell, ≥ 20 periods), E2 free-space evaporation (same seed + sponge boundary), E3 electron destruction test (the Phase-1 winner with coupling off at `t = 0`, with-coupling twin as control), E4 the coupling ladder (`ε ∈ {1 … 0}` scaling the cross force + `(c1, c2)`: minimizer existence + destruction time vs ε); uses the M7.9 orbit-hunting toolkit (E1 as a periodic orbit of the Maxwell flow; E3 as an orbit losing stability); runs after M7.8 and M7.9 | **Theorem 2 from the closure notes** as a known-answer gate (Trkalian at `ω = cλ₀` solves free Maxwell: period, `λ_eff ≈ 1.0` alignment, energy drift at integrator floor); linear-dispersion escape rate + helicity conservation + Arnold bound in E2; the honest boundary (free-space pure Maxwell holds nothing) as a measurement, not an assertion; the Q14 tachyon attributed to the coupling sector via the ladder; results into the [walkthrough](tasks/m7_phase1_walkthrough.md) § 7 (one report: M7.8 + M7.9 + M7.10) |
