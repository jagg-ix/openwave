...
| TaskID | Task title | Description | Validation gate |
| --- | --- | --- | --- |
| [M7.2](tasks/m7_2_fleury_torus.md) | reproduce Fleury on the lattice | **lattice quadrature, no minimizer dependency (can run parallel to M7.1)**: evaluate the paper's toroidal ansatz on the grid, read off `ρ = ε₀∇·E` and `J = (∇×B − c⁻²∂ₜE)/μ₀`, integrate charge / μ / spin / energy under a pinned **conventions contract** (inverted Heaviside, RMS charge, appendix energy convention) + a grid-convergence study; stretch: the **Bessel-envelope** variant (Ceperley `J_m`, Fleury § 5.5's own next step) | recover `Q_rms = e`, `R₀ ≈ 1.573 r_c`, `E₀ ≈ 0.286 E_S`, `U ≈ 0.795 m_e c²`, `ω = 2c/R₀` at demonstrated grid convergence. **M7's "reproduce Faber" trust-builder.** |
| [M7.3](tasks/m7_3_ouroboros_3d.md) | reproduce M6's electron in full 3D | **pre-gate first**: the 3D harmonic functional, restricted to M6's `(α,β)` ansatz, reproduces the M6 ODE **verbatim** (the [`0d_canonical.md § 6`](../../m6_ouroboros/research/0d_canonical.md) graveyard is the cautionary record); then embed the M6 1D profile as a 3D seed, relax at fixed ω, watch for 3D symmetry breaking | recover the **M6 1D BVP value at the SAME calibration** (`H/Q = 1.6890` at `g = 1.0, ω = 1`) from the 3D field to ≤ 1%; Werbos-v5 calibration (`1.6969` at `g = 1.0625`) tracked secondary (Q9); a 3D instability of the 1D profile, if found, is an honest result |
