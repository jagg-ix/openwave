...
| --- | --- | --- | --- |
| [M7.8](tasks/m7_8_helicity_pair.md) | helicity-pair 3:1 test + the Phase 1 walkthrough (Fleury closure) | **GO confirmed at the 2026-07-06 call** (Phase 1 extension). Three deliverables: (a) build the repaired helicity-pair state from the closure notes (CK modes `(m,n,s) = (1,±1,±1)` with `A_r ≠ 0`, LG profile under `λ₀σ = 2`, `w = λ₀σ²`; in lattice units `λ₀ = 1, σ = 2, w = 4`) and run the **amplitude-ratio LADDER** `a₊/a₋ ∈ {1.2 … 2.5}` at fixed `Q_can` (registered caveat: fixed `H_A` partially pins `U₊ − U₋`, so the asymmetry is dialed and the physics question is where the target sits and whether √3 is energetically special); measure `U₊/U₋` (helical k-space decomposition + two cross-checks) and the dimensionless asymmetry `(U₊ − U₋)/(U₊ + U₋)` vs the in-model target 1/2 (the **Q15 directive** `S_z = ℏ/2` under P2, no unit contract needed); (b) [`tasks/m7_phase1_walkthrough.md`](tasks/m7_phase1_walkthrough.md), the under-the-hood report for the author (discovery narrative, equations-as-evolution, numerics + drift evidence, test-suite report, λ_eff defined, the human+AI system, reproduction); (c) closing step: **refresh [`m7_theory_canonical.md`](m7_theory_canonical.md)** to the current METHOD_NOTE format (file:line audit links) and expand it to answer the call's under-the-hood questions (ansatz, integrator, lattice, charge computation) | the closure prediction `U₊/U₋ = 3 + α/2 + 4f_bb`; the measured pair-asymmetry spin vs ℏ/2; the package (data + walkthrough) sent for the author's **adversarial external audit** (his Gemini pass); canonical spec refreshed |
