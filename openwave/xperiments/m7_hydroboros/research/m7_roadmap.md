...
| --- | --- | --- | --- |
| [M7.7](tasks/m7_7_consolidation.md) | consolidate the M7 column (MILESTONE) | fold the winning recipe into a `canonical.md`-style spec + a small auditable physics module (METHOD_REPORT standard); **add HydroBoros (M7) to MODELS.md**; the units-contract decision table; the milestone stop = both comms packages prepared | one runnable canonical script, reproducible first-try; the electron cells land as the new column |
