...
>
> The program runs in five **phases** (renumbered 2026-07-04, extended 2026-07-06 after the Fleury Phase-1-review call): **Phase 1** builds the electron and drafts the coverage column (M7.0-M7.7, ✅ DONE), then **extends through the Fleury collaboration track** (M7.8 helicity pair + walkthrough, M7.9 ChaosBook benchmark, **M7.10-M7.14 reserved** for the pure-Maxwell/Beltrami orbit-hunting work as it firms up post-August); **Phase 2** expands across the forces and the remaining particle sectors (M7.15-M7.20); **Phase 3** publishes the M7 column to MODELS.md for cross-model benchmarking (M7.21, advanced to right after Phase 2: a good publication milestone); **Phase 4** groups the composites still untested in M5 (M7.22); **Phase 5** graduates to production rendering (M7.23). Each task M7.N is an iteration gated against a KNOWN result; every MODELS.md cell (see **Phase 3** below) is assigned to a task. Columns: **taskID · task title · description (the build) · validation gate** (the credibility anchor). Migrated from the implementation plan (now [`m7_background.md`](m7_background.md)) on 2026-07-01.

## IN PROGRESS

| TaskID | Task title | Description | Validation gate |
| --- | --- | --- | --- |
| [M7.8](tasks/m7_8_helicity_pair.md) | helicity-pair 3:1 test + the Phase 1 walkthrough (Fleury closure) | **GO confirmed at the 2026-07-06 call** (Phase 1 extension). Three deliverables: (a) build the repaired helicity-pair state from the closure notes (CK modes `(m,n,s) = (1,±1,±1)` with `A_r ≠ 0`, LG profile under `λ₀σ = 2`, `w = λ₀σ²`), relax at fixed `Q_can`, measure `U₊/U₋` and the pair-asymmetry spin `(U₊ − U₋)/ω` (the **Q15 directive**: target S_z = ℏ/2, the frequency reading is emergent); (b) [`tasks/m7_phase1_walkthrough.md`](tasks/m7_phase1_walkthrough.md), the under-the-hood report for the author (discovery narrative, equations-as-evolution, numerics + drift evidence, test-suite report, λ_eff defined, the human+AI system, reproduction); (c) closing step: **refresh [`m7_theory_canonical.md`](m7_theory_canonical.md)** to the current METHOD_NOTE format (file:line audit links) and expand it to answer the call's under-the-hood questions (ansatz, integrator, lattice, charge computation) | the closure prediction `U₊/U₋ = 3 + α/2 + 4f_bb`; the measured pair-asymmetry spin vs ℏ/2; the package (data + walkthrough) sent for the author's **adversarial external audit** (his Gemini pass); canonical spec refreshed |
...
Reserved IDs for the Fleury-track work as it firms up (his re-entry is post-August): plain **linear Maxwell evolution** (the `m_J = 0, f = 0` sector of the existing engine), Beltrami / variable-λ ansatz as seeds, a defined notion of a stable orbit in the field flow (the ChaosBook methodology from M7.9), and the static→dynamic charge transition he plans to lead. Two-track principle (2026-07-06): the Maxwell track runs alongside the harmonic/functional track that produced Phase 1, on the same lattice; measurements adjudicate. Honest boundary, stated up front: pure free-space linear Maxwell cannot hold a localized finite-energy state indefinitely (Nadirashvili; our Q11 dispersion work), so this track searches and verifies with the variable-λ profile as the localizer, and what sustains that profile (back-reaction) remains the open physics.
