...
> **M7 is PARKED (2026-07-07, post-M7.10)** until the author returns from the conference and reviews the [walkthrough § 7](tasks/m7_phase1_walkthrough.md) package (M7.8 + M7.9 + M7.10); his adversarial pass may re-scope the band below before anything runs. The sequencing rationale lives in the future task docs themselves ([M7.11 § 0](tasks/m7_11_variable_lambda.md) carries the full post-M7.10 analysis; [M7.15](tasks/m7_15_magnetic.md)/[M7.16](tasks/m7_16_gravity.md)/[M7.19](tasks/m7_19_lepton_family.md)/[M7.20](tasks/m7_20_dark_matter.md) each carry their own planning note).

| TaskID | Task title | Description | Validation gate |
| --- | --- | --- | --- |
| [M7.11](tasks/m7_11_variable_lambda.md) | The variable-λ vacuum-cure check (Q14) | the two computable cure candidates from the M7.10 constraint (the bilinear carries BOTH the tachyon and the confinement, so a cure must fix `k → 0` while keeping soliton-scale binding): (a) linearize the vacuum WITH the scalar/Gauss sector (does it lift `det M(0) = −ε_x²`?); (b) the author's variable-λ mass profile as a fixed background (`ε_x(r)` canonical inside, `→ 0` outside): bound interior + massless healthy exterior; plus the first ISOLATED-orbit hunt (`find_cycle` period recovery, non-degenerate once the flow is inhomogeneous; the M7.10 non-isolation demo is the null reference) | exterior vacuum noise does NOT grow while a localized minimizer (r50 ~ 3.4, the M7.10 ε = 1 anchor) survives ≥ 10 T on the profile, or the failure mode documented; the scalar-sector linearization decided analytically + lattice cross-checked; genuine `find_cycle` period recovery = the first isolated orbit of the program. Unblocks M7.18 (all real-time physics) |

## PHASE 1 EXTENSION, reserved band (M7.11-M7.14): the pure-Maxwell / Beltrami orbit track

**M7.10 is DELIVERED** (2026-07-07: the no-Lagrangian test, row in the author-collaboration DONE table below) and **M7.11 is now defined** (the variable-λ vacuum-cure check, § BACKLOG above, top = next); M7.12-M7.14 stay reserved for the Fleury-track work as it firms up (his re-entry is post-August): plain **linear Maxwell evolution** (the `m_J = 0, f = 0` sector of the existing engine, bootstrapped by M7.10), Beltrami / variable-λ ansatz as seeds, a defined notion of a stable orbit in the field flow (the ChaosBook methodology from M7.9), and the static→dynamic charge transition he plans to lead. Two-track principle (2026-07-06): the Maxwell track runs alongside the harmonic/functional track that produced Phase 1, on the same lattice; measurements adjudicate. Honest boundary, stated up front: pure free-space linear Maxwell cannot hold a localized finite-energy state indefinitely (Nadirashvili; our Q11 dispersion work; M7.10 turns this into a measurement), so this track searches and verifies with the variable-λ profile as the localizer, and what sustains that profile (back-reaction) remains the open physics.

## PHASE 2, forces and the remaining particle sectors (M7.15-M7.20)

| TaskID | Task title | Description | Validation gate |
| --- | --- | --- | --- |
| [M7.15](tasks/m7_15_magnetic.md) | magnetic force | the per-defect magnetic structure carried by the electron's clock (Coulomb already landed in M7.4/M7.6) | magnetic force from the clock's `Γ₀` (pure twist is EM-silent; the M5 mechanism) |
| [M7.16](tasks/m7_16_gravity.md) | gravity | the time-axis boost of the field (the M5 4×4 route) | a GEM coupling that vanishes at zero boost; honest pass / fail (Ouroboros stops before gravity, so this is genuinely hard for M7) |
