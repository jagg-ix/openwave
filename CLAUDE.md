...
| [Method Report](/dev_docs/METHOD_REPORT.md) | **MANDATORY** standard for any report/email to a model's theory owner or external physicist |

### Method report (model-owner-facing output): MANDATORY

Any report, summary doc, or email addressed to a model's theory owner (M5: Duda; M7: Fleury; any advisor) follows [`dev_docs/METHOD_REPORT.md`](/dev_docs/METHOD_REPORT.md): **equations first** (Hamiltonian / potential / observable definitions in math notation), an **equation-to-code map** with commit-pinned GitHub permalinks (`#L` anchors), the functional in a **small auditable module**, results after methods, a minimal physics-first inspection set. Adopted 2026-07-03 after the M5.16 audit failure ("still I have no idea what does it calculate"): results a physicist cannot audit by reading carry no weight, regardless of correctness.
